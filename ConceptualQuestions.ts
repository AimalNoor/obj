...
let number=5
if(number >3 ){console.log("yes")}
else{console.log("no")}

//question6

function calculatearea (radius:number,pie:number=22/7)
{return(pie *radius**2)
}
...
function PositiveCount(Numbers: number[]){
    ...
    function Words(words:string[]):string[]{
    ...
    //Q13
    function squaredNumbers(num: number[]): number[] {
...
function reverseArray(mob: number[]): number[] {
...
let reversed: number[] = reverseArray(myArray)
...
function Index(arr: any[], element: any): number {
...
function countOccurrences(numbers: number[], numberToCount: number): number {
    let count = 0
...
let names1 = ['Aimal', 'Wania', 'Widaad']
let names2 = ['Widaad', 'Wania']
let names3 = ['Aimal']
...
function fahrenheitToCelsius(fTemps: number[]): number[] {
...
function reverseArray2(mob: number[]): number[] {
...
let myArray22: number[] = [1, 2, 3, 4, 5]
let reversed2: number[] = reverseArray(myArray)
console.log(reversed) 
